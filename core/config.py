...
class QueryLoop(_Base):
    ...
    #: 從第幾個條件開始查（含，從 1 算起）。
    #:
    #: 跟頁碼的「從第幾頁開始」是同一件事。97 個商品分類跑完要好幾個小時，
    #: 使用者本來就會分次跑——今天跑 1 到 20，明天從 21 接下去。沒有這一格
    #: 的話，想爬第 7 個分類的唯一辦法是從第 1 個開始重跑。
    #:
    #: 跟中斷續跑不衝突：這一格是使用者指定的起點，續跑的進度是在這個起點
    #: 之後往前推的。
    start_at: int = Field(default=1, ge=1)
    #: 起點與終點也可以直接寫選項上的文字，例如「03 魚類」到「10」。
    #:
    #: 這才是使用者實際的說法——他看著那個下拉選單，說的是「我要爬 03 到 10」，
    #: 沒有人會去數那是第幾個。填了文字就以文字為準，對不到任何選項時退回
    #: 序號並在日誌裡說明，不會安靜地爬成別的範圍。
    start_value: str = ""
    end_value: str = ""
    #: 每一次查詢之後等多久（毫秒）讓結果出現。
    wait_ms: int = Field(default=1200, ge=0, le=30_000)
...
class CompletionSection(_Base):
    ...
    #: 在一家公司的網站上最多讀幾頁（首頁算一頁）。
    #:
    #: **預設 3 是量出來的，不是猜的，調高之前先看這一段。** 拿六家真實的
    #: 台灣公司網站比過 3 頁與 5 頁：
    #:
    #:     max_pages=3   信箱 0/6
    #:     max_pages=5   信箱 0/6，其中兩家各多送 2 次請求
    #:
    #: 一次都沒多找到。原因是抓不到信箱的網站**根本沒有刊登信箱**（只放聯絡
    #: 表單），多翻幾頁翻不出不存在的東西；真的有刊登的多半就在首頁頁尾或
    #: 第一層的「聯絡我們」，3 頁本來就夠。
    #:
    #: 所以這個值開放出來是給「知道自己的名單長什麼樣」的人調的，不是預設
    #: 該調高的東西——調高的確定代價是每一家多送請求，收益不確定。
    max_pages: int = Field(default=3, ge=1, le=10)

    #: 畫面上「這次補幾家」那個輸入框記住的值。
    #:
    #: 不是上限、也不是效能參數，就是使用者上次輸入的數字。放在設定裡是為了
    #: 讓它跨次啟動還在——分批補一份 2700 家的名單要按很多次，每次都要重打
    #: 一遍數字很煩。實際跑幾家一律以畫面上當下的值為準。
    batch_size: int = Field(default=200, ge=1, le=100_000)

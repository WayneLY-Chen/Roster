...
# 印出貓跟狗。
#
# 每一支會開終端機的腳本都在最一開始印同一張圖（assets/pets.txt），三個平台
# 共用同一份檔案——分開放的話改一次要記得改六個地方。
#
# 圖是純 ASCII 的，那不是美術上的選擇：Windows 預設的主控台字型
# （Lucida Console、Consolas）沒有全形與假名的字形，UTF-8 的圖在那裡會整張
# 消失。同一份檔案要在三個平台都印得出來，就只能用每個字型都有的字。
print_pets() {
    local root="${1:-.}"
    printf "\n"
    cat "$root/assets/pets.txt" 2>/dev/null || true
}

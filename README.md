...
「爬取」頁的「**要收集哪些欄位**」可以取消勾選不需要的項目。沒勾的欄位在寫入
資料庫前就會被清空——只想要公司名稱與信箱時，多抓回來的地址與統編只是雜訊，
還會讓「尋找重複」的比對多出無謂的維度。
...
台灣的名錄網站幾乎都是「一個分類一個頁面」——工業會依公會分類、iyp 依產業
分類。分類本身就是產業，但它寫在麵包屑或頁面標題裡、不在每一列的資料中，
所以逐列抓的欄位規則抓不到它。

在來源設定加上 `default_industry`，頁面沒提供產業時就用它補上：

```yaml
- name: "iyp-精密機械"
  type: "generic_html"
  default_industry: "精密機械"      # 頁面自己有寫產業時不會被覆蓋
  ...
```

...
class Company(Base):
    ...
    #: 最後一次跑過「補齊公司資料」的時間，不管有沒有補到東西。
    #:
    #: 這一欄是分批補齊的書籤。使用者輸入「這次跑 200 家」時，
    #: :func:`crawler.complete.complete_companies` 就是照這一欄由舊到新排序、
    #: 沒跑過的排最前面，取前 200 家——所以下一次按下去自動接著上一次跑，
    #: 使用者不必記得上次跑到第幾家。
    #:
    #: 沒有這一欄的話，「還缺欄位」是唯一的篩選條件，而補不到東西的公司永遠
    #: 都還缺欄位：每一批都會是同一批，2699 家的名單會永遠停在前 200 家。
    #:
    #: 「試過但沒補到」也一定要蓋章，這是這一欄唯一的重點。真的補不到的
    #: 公司（官網只放表單、沒有公開信箱）才是最需要被推到隊伍後面的那一批。
    #:
    #: 刻意不加索引：排序是在 Python 端做的（``complete.py`` 挑目標時要一起
    #: 看加密欄位缺不缺，SQL 看不懂那些），所以索引一次都用不到。而且
    #: ``database/migrate.py`` 只會 ``ADD COLUMN``、不會補索引，加了也只有
    #: 全新的資料庫才有——那種只在一半機器上存在的東西比沒有更糟。
    completion_checked_at: Mapped[datetime | None] = mapped_column(DateTime)
